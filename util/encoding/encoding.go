...
func DecodeString(str string) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.New("encoding.DecodeString: " + err.Error())
	}

	bytesLen := len(buf)
	if bytesLen < checksumLen {
		return nil, errors.New("encoding.DecodeString: no checksum bytes")
	}
	// Verify the checksum
	rawBytes := buf[:bytesLen-checksumLen]
	checksum := buf[bytesLen-checksumLen:]
	if !bytes.Equal(checksum, Sum256(rawBytes)[:checksumLen]) {
		return nil, errors.New("encoding.DecodeString: invalid input checksum")
	}
	return rawBytes, nil
}
...
func DecodeQuoteString(str string) ([]byte, error) {
	...
	if strLen < 2 || str[0] != '"' || str[strLen-1] != '"' {
		return nil, errors.New("encoding.DecodeQuoteString: invalid quote string")
	}
...
func DecodeStringWithLen(str string, expectedLen int) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.New("encoding.DecodeStringWithLen: " + err.Error())
	}
	if bytesLen := len(buf); bytesLen != expectedLen {
		return nil, errors.New("encoding.DecodeStringWithLen: invalid bytes length, expected " +
...
func DecodeQuoteStringWithLen(str string, expectedLen int) ([]byte, error) {
	strLen := len(str)
	if strLen < 2 || str[0] != '"' || str[strLen-1] != '"' {
		return nil, errors.New("encoding.DecodeQuoteStringWithLen: invalid quote string")
	}
