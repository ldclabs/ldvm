...
func (tx *TxUpdateData) MarshalJSON() ([]byte, error) {
	...
	v := tx.ld.Copy()
	errp := util.ErrPrefix("transactions.TxUpdateData.MarshalJSON: ")
	if tx.input == nil {
...
func (tx *TxUpdateData) SyntacticVerify() error {
	var err error
	errp := util.ErrPrefix("transactions.TxUpdateData.SyntacticVerify: ")
...
func (tx *TxUpdateData) Apply(ctx ChainContext, cs ChainState) error {
	var err error
	errp := util.ErrPrefix("transactions.TxUpdateData.Apply: ")
