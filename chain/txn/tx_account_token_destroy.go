...
func (tx *TxDestroyToken) SyntacticVerify() error {
	var err error
	errp := erring.ErrPrefix("txn.TxDestroyToken.SyntacticVerify: ")
	...
	if token := ids.TokenSymbol(tx.ld.Tx.From); !token.Valid() {
...
func (tx *TxDestroyToken) Apply(ctx ChainContext, cs ChainState) error {
	var err error
	errp := erring.ErrPrefix("txn.TxDestroyToken.Apply: ")
