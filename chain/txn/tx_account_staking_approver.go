...
func (tx *TxUpdateStakeApprover) MarshalJSON() ([]byte, error) {
	...
	v := tx.ld.Copy()
	errp := erring.ErrPrefix("txn.TxUpdateStakeApprover.MarshalJSON: ")
	if tx.input == nil {
...
func (tx *TxUpdateStakeApprover) SyntacticVerify() error {
	var err error
	errp := erring.ErrPrefix("txn.TxUpdateStakeApprover.SyntacticVerify: ")
...
func (tx *TxUpdateStakeApprover) Apply(ctx ChainContext, cs ChainState) error {
	var err error
	errp := erring.ErrPrefix("txn.TxUpdateStakeApprover.Apply: ")
