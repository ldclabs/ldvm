...
func (tx *TxPunish) MarshalJSON() ([]byte, error) {
	...
	v := tx.ld.Copy()
	errp := erring.ErrPrefix("txn.TxPunish.MarshalJSON: ")
	if tx.input == nil {
...
func (tx *TxPunish) SyntacticVerify() error {
	var err error
	errp := erring.ErrPrefix("txn.TxPunish.SyntacticVerify: ")
	...
	switch {
	case tx.ld.Tx.From != ids.GenesisAccount:
...
func (tx *TxPunish) Apply(ctx ChainContext, cs ChainState) error {
	var err error
	errp := erring.ErrPrefix("txn.TxPunish.Apply: ")
