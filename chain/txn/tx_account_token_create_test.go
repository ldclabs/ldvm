...
func TestTxCreateToken(t *testing.T) {
	...
	itx, err = NewTx(ltx)
	require.NoError(t, err)
	senderAcc.Add(ids.NativeToken, new(big.Int).SetUint64(10000000000000))
	cs.CommitAccounts()
	assert.ErrorContains(itx.Apply(ctx, cs),
	...
	assert.Equal(uint64(10000000000000), tokenAcc.BalanceOfAll(ids.NativeToken).Uint64())
	...
	assert.Equal(unit.LDC*10, tokenAcc.LD().MaxTotalSupply.Uint64())
	assert.Equal(unit.LDC*10, tokenAcc.LD().Tokens[token.AsKey()].Uint64())
...
func TestTxCreateTokenGenesis(t *testing.T) {
	...
	ldcAcc := cs.MustAccount(ids.LDCAccount)
	assert.Equal(ctx.ChainConfig().MaxTotalSupply.Uint64(),
		ldcAcc.Balance().Uint64())
	assert.Equal(ctx.ChainConfig().MaxTotalSupply.Uint64(),
		ldcAcc.BalanceOfAll(ids.NativeToken).Uint64())
