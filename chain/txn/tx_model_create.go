...
func (tx *TxCreateModel) MarshalJSON() ([]byte, error) {
	...
	v := tx.ld.Copy()
	errp := erring.ErrPrefix("txn.TxCreateModel.MarshalJSON: ")
	if tx.input == nil {
...
func (tx *TxCreateModel) SyntacticVerify() error {
	var err error
	errp := erring.ErrPrefix("txn.TxCreateModel.SyntacticVerify: ")
	...
	tx.input.ID = ids.ModelIDFromHash(tx.ld.ID)
	return nil
}
...
func (tx *TxCreateModel) ApplyGenesis(ctx ChainContext, cs ChainState) error {
	var err error
	errp := erring.ErrPrefix("txn.TxCreateModel.ApplyGenesis: ")
	...
	if len(tx.input.Keepers) > 0 {
		modelAcc, err := cs.LoadAccount(ids.Address(tx.input.ID))
		if err != nil {
			return errp.ErrorIf(err)
		}
		if !modelAcc.IsEmpty() {
...
func (tx *TxCreateModel) Apply(ctx ChainContext, cs ChainState) error {
	var err error
	errp := erring.ErrPrefix("txn.TxCreateModel.Apply: ")
