...
func (tx *TxUpdateNonceTable) MarshalJSON() ([]byte, error) {
	...
	v := tx.ld.Copy()
	errp := erring.ErrPrefix("txn.TxUpdateNonceTable.MarshalJSON: ")
	if tx.input == nil {
...
func (tx *TxUpdateNonceTable) SyntacticVerify() error {
	var err error
	errp := erring.ErrPrefix("txn.TxUpdateNonceTable.SyntacticVerify: ")
	...
	if err = encoding.UnmarshalCBOR(tx.ld.Tx.Data, &tx.input); err != nil {
...
func (tx *TxUpdateNonceTable) Apply(ctx ChainContext, cs ChainState) error {
	var err error
	errp := erring.ErrPrefix("txn.TxUpdateNonceTable.Apply: ")
