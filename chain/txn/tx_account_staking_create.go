...
func (tx *TxCreateStake) MarshalJSON() ([]byte, error) {
	...
	v := tx.ld.Copy()
	errp := erring.ErrPrefix("txn.TxCreateStake.MarshalJSON: ")
	if tx.input == nil {
...
func (tx *TxCreateStake) SyntacticVerify() error {
	var err error
	errp := erring.ErrPrefix("txn.TxCreateStake.SyntacticVerify: ")
...
func (tx *TxCreateStake) Apply(ctx ChainContext, cs ChainState) error {
	var err error
	errp := erring.ErrPrefix("txn.TxCreateStake.Apply: ")
