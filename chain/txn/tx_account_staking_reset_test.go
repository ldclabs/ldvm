...
func TestTxResetStake(t *testing.T) {
	...
	input := &ld.StakeConfig{
		...
		WithdrawFee: 1_000,
		MinAmount:   new(big.Int).SetUint64(unit.LDC * 100),
		MaxAmount:   new(big.Int).SetUint64(unit.LDC * 100),
	}
	ltx = &ld.Transaction{Tx: ld.TxData{
		Type:      ld.TypeResetStake,
	...
	assert.Equal(unit.LDC*100, stakeAcc.LD().Stake.MinAmount.Uint64())
	assert.Equal(unit.LDC*100, stakeAcc.LD().Stake.MaxAmount.Uint64())
