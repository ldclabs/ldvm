...
func TestTxDestroyStake(t *testing.T) {
	...
	keeperAcc.Add(ids.NativeToken,
		new(big.Int).SetUint64(ctx.FeeConfig().MinStakePledge.Uint64()+unit.LDC*2))
	...
	stakeAcc.Add(ids.NativeToken, new(big.Int).SetUint64(unit.LDC))
	cs.CommitAccounts()
	assert.ErrorContains(itx.Apply(ctx, cs),
	...
	senderAcc.Add(ids.NativeToken, new(big.Int).SetUint64(unit.LDC*12))
	assert.NoError(itx.Apply(ctx, cs))
	...
	assert.Equal(ctx.FeeConfig().MinStakePledge.Uint64()+unit.LDC*11,
		stakeAcc.BalanceOfAll(ids.NativeToken).Uint64())
	...
	input2 = &ld.TxTransfer{Amount: new(big.Int).SetUint64(unit.LDC * 10)}
	ltx = &ld.Transaction{Tx: ld.TxData{
	...
	withdrawFee := unit.LDC * 10 * scfg.WithdrawFee / 1_000_000
	assert.Equal(unit.LDC*11-withdrawFee-senderGas*(ctx.Price+100),
		senderAcc.Balance().Uint64())
	assert.Equal(unit.LDC+withdrawFee, stakeAcc.Balance().Uint64())
	...
	assert.Equal(ctx.FeeConfig().MinStakePledge.Uint64()+unit.LDC*2+withdrawFee-(keeperGas+stakeGas)*(ctx.Price+100),
		keeperAcc.Balance().Uint64())
	...
	ltx = &ld.Transaction{Tx: ld.TxData{
		...
		From:      keeper,
		To:        &stakeid,
		Amount:    new(big.Int).SetUint64(ctx.FeeConfig().MinStakePledge.Uint64() + unit.LDC),
		Data:      sinput.Bytes(),
	}}
	assert.NoError(ltx.SignWith(signer.Signer2))
	assert.NoError(ltx.SyntacticVerify())
	...
	assert.Equal(ctx.FeeConfig().MinStakePledge.Uint64()+unit.LDC,
		stakeAcc.BalanceOfAll(ids.NativeToken).Uint64())
	...
	stakeAcc.Add(token, new(big.Int).SetUint64(unit.LDC*9))
	ltx = &ld.Transaction{Tx: ld.TxData{
		Type:      ld.TypeDestroyStake,
		ChainID:   ctx.ChainConfig().ChainID,
		Nonce:     1,
...
func TestTxDestroyStakeWithApproverAndLending(t *testing.T) {
	...
	keeperAcc := cs.MustAccount(keeper)
	keeperAcc.Add(ids.NativeToken,
		new(big.Int).SetUint64(ctx.FeeConfig().MinStakePledge.Uint64()+unit.LDC*3))
	assert.NoError(itx.Apply(ctx, cs))
	...
	approverAcc.Add(ids.NativeToken, new(big.Int).SetUint64(unit.LDC))
	assert.NoError(itx.Apply(ctx, cs))
	...
	assert.Equal(unit.LDC*9, stakeAcc.BalanceOf(token).Uint64())
	assert.Equal(unit.LDC, approverAcc.BalanceOf(token).Uint64())
	...
	assert.Equal((keeperGas+stakeGas+approverGas)*100,
		itx.(*TxDestroyStake).miner.Balance().Uint64())
	assert.Equal(ctx.FeeConfig().MinStakePledge.Uint64()+unit.LDC*2-(keeperGas+stakeGas)*(ctx.Price+100),
		keeperAcc.Balance().Uint64())
	assert.Equal(unit.LDC*0, stakeAcc.BalanceOf(token).Uint64())
	assert.Equal(unit.LDC*10, keeperAcc.BalanceOf(token).Uint64())
