...
func (tx *TxExchange) MarshalJSON() ([]byte, error) {
	...
	v := tx.ld.Copy()
	errp := erring.ErrPrefix("txn.TxExchange.MarshalJSON: ")
	if tx.input == nil {
...
func (tx *TxExchange) SyntacticVerify() error {
	var err error
	errp := erring.ErrPrefix("txn.TxExchange.SyntacticVerify: ")
	...
	tx.quantity = new(big.Int).SetUint64(unit.LDC)
	...
	case tx.quantity.Cmp(tx.input.Minimum) < 0:
		...
		min.Quo(min, new(big.Int).SetUint64(unit.LDC))
	...
	case tx.quantity.Cmp(tx.input.Quota) > 0:
		...
		max.Quo(max, new(big.Int).SetUint64(unit.LDC))
		return errp.Errorf("invalid amount, expected <=%v, got %v",
			max, tx.ld.Tx.Amount)
...
func (tx *TxExchange) Apply(ctx ChainContext, cs ChainState) error {
	var err error
	errp := erring.ErrPrefix("txn.TxExchange.Apply: ")
